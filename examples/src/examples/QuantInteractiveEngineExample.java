...
public class QuantInteractiveEngineExample {
        ...
        try {
            ...
            switch(response.getStatusCode()) {
                ...
                case 201:
                    ...
                    result.renameTo(new File("output.txt")); // Ensure that file with the same name does not exist in the location
                    System.out.println("Result file : output.txt");
                ...
                case 202:
                    ...
                    for (Map.Entry<String, CalculationUnitStatus> calculationUnitParameters : getStatus.getData().getData().getUnits().entrySet()) {
                        if (calculationUnitParameters.getValue().getStatus() == CalculationUnitStatus.StatusEnum.SUCCESS)
                        {
                            ...
                            result.renameTo(new File("output.txt")); // Ensure that file with the same name does not exist in the location
                            System.out.println("Result file :output.txt");
                            //outputCalculationResult(result,headers);
                            resultResponse = apiInstance.getCalculationUnitInfoByIdWithHttpInfo(id, unitId);
                            result =  resultResponse.getData();
                            System.out.println("Calculation Info");
                            result.renameTo(new File("outputInfo.txt")); // Ensure that file with the same name does not exist in the location
                            System.out.println("Result file : outputInfo.txt");
                            //outputCalculationResult(result,headers);
                        }
                    }
                    break;
    ...
    private static void outputCalculationResult(File result, Map<String, List<String>> headers) {
        try {
